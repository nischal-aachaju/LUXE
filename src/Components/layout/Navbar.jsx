  ...
  return (
    <div className=' sticky top-0 h-14 bg-white  w-full z-100'>
      <div className=' h-14  w-fill flex justify-between px-4 shadow-[0px_-6px_10px_0px_#525252]'>
        ...
        {menuBox ? null : <nav className=' absolute h-10 w-full top-14 backdrop-blur-sm left-0  overflow-hidden z-100 flex items-center gap-0 '>
          {nav.map((elem) => {
            return <a className='bg-white/10 text-white  w-full  overflow-hidden text-shadow-[0px_0px_4px_#000000] border-y  h-14 text-center pt-5  text-[11px] font-extrabold  underline ' href="">{elem}</a>
          })}
        </nav>}
      </div>
    </div>
